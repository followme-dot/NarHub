...
import {
  ArrowLeft, Play, Shield, Code2, CheckCircle2,
  Users, Lock, TrendingUp, FileText, CreditCard, Copy, Check
} from 'lucide-react'
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <p className="text-white/80 mb-8 leading-relaxed max-w-xl">
                High-performance liquidity monitoring and settlement engine with real-time reconciliation.
                Multi-entity position management with automated risk limit enforcement and complete audit trails.
              </p>

              <div className="mb-8">
                <span className="text-white/70 text-sm font-medium uppercase tracking-wider">Valuation</span>
                <p className="text-4xl md:text-5xl font-bold text-white mt-2">{formatPrice(product.priceMin)}</p>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              { title: 'Full Audit', description: 'Complete audit trail system' }
            ].map((feature, index) => (
        ...
        <div className="max-w-4xl mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-lg text-[#4a4a68] leading-relaxed mb-8">
              INFRA VAULT CORE is the backbone of financial operations, providing real-time liquidity monitoring,
              automated settlement processing, and multi-entity reconciliation. Built with Rust and Go for maximum
              performance, it handles thousands of transactions per second with sub-50ms latency.
            </p>

            <div className={`bg-gradient-to-r ${product.gradient.from} ${product.gradient.to} rounded-2xl p-8 text-white mb-8`}>
              <h3 className="text-xl font-bold mb-4 flex items-center gap-2"><TrendingUp className="w-6 h-6" />Core Features</h3>
              <p className="leading-relaxed opacity-95">
                Real-time liquidity monitoring across all accounts, automated settlement with risk limit enforcement,
                multi-entity position management, and comprehensive audit trails for regulatory compliance.
              </p>
            </div>
          </motion.div>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            <div className="bg-white rounded-2xl p-6 border border-gray-200">
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${product.gradient.from} ${product.gradient.to} flex items-center justify-center`}><Code2 className="w-6 h-6 text-white" /></div>
                <h3 className="text-xl font-bold text-[#1a1a2e]">Complete Source Code</h3>
              </div>
              <p className="text-[#4a4a68]">65,000+ lines of Rust, Go, and TypeScript with TimescaleDB time-series storage.</p>
            </div>

            <div className="bg-white rounded-2xl p-6 border border-gray-200">
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${product.gradient.from} ${product.gradient.to} flex items-center justify-center`}><Shield className="w-6 h-6 text-white" /></div>
                <h3 className="text-xl font-bold text-[#1a1a2e]">Full IP Transfer</h3>
              </div>
              <p className="text-[#4a4a68]">100% ownership with perpetual license. No restrictions on deployment.</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowPayment(false)} className="p-2 rounded-lg hover:bg-gray-100"><span className="text-2xl">×</span></button>
            </div>

            <div className="flex gap-2 mb-6">
              <button onClick={() => setIsOutsideEU(false)} className={`flex-1 py-3 px-4 rounded-xl font-medium transition-all ${!isOutsideEU ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>SEPA (EU)</button>
              <button onClick={() => setIsOutsideEU(true)} className={`flex-1 py-3 px-4 rounded-xl font-medium transition-all ${isOutsideEU ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-600'}`}>SWIFT (International)</button>
            </div>
            ...
            {!isOutsideEU && (
              <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 mb-6">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between items-center"><span className="text-blue-700">IBAN:</span><div className="flex items-center gap-2"><span className="font-mono font-medium text-blue-900">{bankDetails.iban}</span><button onClick={copyIban} className="p-1 rounded hover:bg-blue-100">{copiedIban ? <Check className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4 text-blue-600" />}</button></div></div>
            ...
            {isOutsideEU && (
              <div className="bg-purple-50 border border-purple-200 rounded-2xl p-6 mb-6">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between items-center"><span className="text-purple-700">IBAN:</span><div className="flex items-center gap-2"><span className="font-mono font-medium text-purple-900">{bankDetails.iban}</span><button onClick={copyIban} className="p-1 rounded hover:bg-purple-100">{copiedIban ? <Check className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4 text-purple-600" />}</button></div></div>
      ...
      <section className="py-20">
        <div className="max-w-4xl mx-auto px-4">
          <div className={`bg-gradient-to-r ${product.gradient.from} ${product.gradient.to} rounded-3xl p-8 md:p-12 text-white text-center`}>
            <h2 className="text-3xl md:text-4xl font-bold mb-4">Ready to Power Your Settlement Engine?</h2>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {isAuthenticated ? (
                <Button size="lg" onClick={() => setShowPayment(true)} className="bg-white text-gray-900 hover:bg-gray-100 shadow-xl"><CreditCard className="w-5 h-5 mr-2" /> View Payment Details</Button>
              ) : (
                <Link href="/login?callbackUrl=/products/infra-vault-core"><Button size="lg" className="bg-white text-gray-900 hover:bg-gray-100 shadow-xl"><Lock className="w-5 h-5 mr-2" /> Login to Purchase</Button></Link>
              )}
